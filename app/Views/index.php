...
<?php if ($generalSettings->show_featured_section == 1): ?>
    <?= view('post/_featured_posts', $featuredPosts); ?>
<?php endif; ?>
                ...
                <div class="col-sm-12 news-ticker-cnt">
                    <div class="row m-0">
                        ...
                        <div class="news-ticker-btn-cnt">
                            <a href="javascript:void(0)" id="btn_newsticker_prev" class="bnt-news-ticker news-prev"><span class="no-text">prev</span><span class="icon-arrow-left"></span></a>
                            <a href="javascript:void(0)" id="btn_newsticker_next" class="bnt-news-ticker news-next"><span class="no-text">next</span><span class="icon-arrow-right"></span></a>
                        </div>
                    </div>
                </div>
            <?php else: ?>
                    ...
                    foreach ($baseCategories as $category):
                        if ($category->show_at_homepage == 1 && $category->lang_id == $activeLang->id):
                            if ($category->block_type == "block-1") {
                                echo view('partials/_category_block_type_1', ['category' => $category]);
                            }
                            if ($category->block_type == "block-2") {
                                echo view('partials/_category_block_type_2', ['category' => $category]);
                            }
                            if ($category->block_type == "block-3") {
                                echo view('partials/_category_block_type_3', ['category' => $category]);
                            }
                            if ($category->block_type == "block-4") {
                                echo view('partials/_category_block_type_4', ['category' => $category]);
                            }
                            if ($category->block_type == "block-5") {
                                echo view('partials/_category_block_type_5', ['category' => $category]);
                            }
                            if ($x == 0) {
                                echo view('partials/_ad_spaces', ['adSpace' => 'index_top', 'class' => 'bn-p-b']);
                            }
                            $x++;
                        endif;
                    endforeach;
                endif;
                echo view('partials/_ad_spaces', ['adSpace' => 'index_bottom', 'class' => 'bn-p-b']);
                                    ...
                                    <div class="section-content">
                                        <div class="row latest-articles">
                                            <div id="last_posts_content">
                                                <?= view('post/_index_latest_posts', ['limitLoadMorePosts' => $limitLoadMorePosts]); ?>
                                            </div>
                                        </div>
                                    </div>
