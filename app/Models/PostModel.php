...
class PostModel extends BaseModel
{
    ...
    public function getFeaturedPosts()
    {
        ...
        if ($this->generalSettings->sort_slider_posts == 'by_featured_order') {
            $this->builder->orderBy('posts.featured_order, posts.id');
        } else {
            $this->builder->orderBy('posts.created_at DESC');
        }
        return $this->builder->get(4)->getResult();
    }
    ...
    //get popular posts
    public function getPopularPosts($dateType, $langId)
    {
        $this->buildQuery($langId);
        if ($dateType == 'weekly') {
            $this->builder->join('(SELECT post_pageviews_week.post_id, COUNT(*) AS count FROM post_pageviews_week GROUP BY post_pageviews_week.post_id ORDER BY count DESC, post_pageviews_week.post_id LIMIT 10) AS hit_counts', 'hit_counts.post_id = posts.id');
        } elseif ($dateType == 'monthly') {
            $this->builder->join('(SELECT post_pageviews_month.post_id, COUNT(*) AS count FROM post_pageviews_month GROUP BY post_pageviews_month.post_id ORDER BY count DESC, post_pageviews_month.post_id LIMIT 10) AS hit_counts', 'hit_counts.post_id = posts.id');
        } else {
            return $this->builder->orderBy('pageviews DESC, posts.id')->get($this->popularPostsLimit)->getResult();
        }
        return $this->builder->select('hit_counts.count AS pageviews_count')->orderBy('count DESC, posts.id')->get($this->popularPostsLimit)->getResult();
    }
    ...
    {
        return $this->builder->join('categories', 'categories.id = posts.category_id')->join('users', 'users.id = posts.user_id')->where('posts.is_scheduled', 0)
            ->where('posts.visibility', 1)->where('posts.status', 1)->countAllResults();
    }
    ...
    {
        return $this->builder->where('posts.id <', cleanNumber($id))->where('posts.is_scheduled = 0')->where('posts.visibility = 1')->where('posts.status = 1')->where('posts.lang_id', cleanNumber($this->activeLang->id))->orderBy('posts.created_at DESC')->get(1)->getRow();
    }
        ...
        if (!empty($post) && !empty($author) && !$agent->isRobot() && !isBot()) {
            if (empty(helperGetCookie('post_' . $post->id))) {
                ...
                if ($this->generalSettings->reward_system_status == 1 && !empty($this->generalSettings->reward_amount) && $author->reward_system_enabled == 1) {
                    $rewardAmount = number_format($this->generalSettings->reward_amount / 1000, 5, '.', '');
                }
                $row = $this->db->table('post_pageviews_week')->where('post_id', $post->id)->where('ip_address', $ipAddress)->where('user_agent', $userAgent)->get(1)->getRow();
                if (empty($row)) {
                    if ($this->builder->where('id', $post->id)->update(['pageviews' => $post->pageviews + 1])) {
                        ...
                        if($this->db->table('post_pageviews_week')->insert($data)){
                            $this->db->table('post_pageviews_month')->insert($data);
                            if ($rewardAmount > 0) {
                                $this->db->query("UPDATE users SET balance = FORMAT(balance + ?, 5), total_pageviews = total_pageviews + 1 WHERE id = ?", array($rewardAmount, $post->user_id));
                            }
                        }
                    }
                }
