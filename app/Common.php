...
if (!function_exists('getFavicon')) {
    function getFavicon()
    {
        ...
        if (!empty($generalSettings)) {
            ...
            return base_url("assets/img/pwa/favicon.ico");
        }
        return base_url("assets/img/pwa/favicon.ico");
    }
}
...
if (!function_exists('removeForbiddenCharacters')) {
  function removeForbiddenCharacters($str) {
    if(is_array($str)){
      foreach($str as $k=>$v)
      {
        $str[$k] = removeForbiddenCharacters($v);
      }
      return $str;
    }else{
      $str = trim($str);
    ...
    function getAdminNavItemEditLink($menuItem)
    {
        if (!empty($menuItem)) {
            if ($menuItem->item_type == "category") {
                if ($menuItem->item_parent_id == 0) {
                    return adminUrl('edit-category/' . $menuItem->item_id . '?redirect_url=' . current_url() . '?' . $_SERVER['QUERY_STRING']);
                } else {
                    return adminUrl('edit-subcategory/' . $menuItem->item_id . '?redirect_url=' . current_url() . '?' . $_SERVER['QUERY_STRING']);
                }
            } else {
        ...
        if (!empty($menuItem)) {
            if ($menuItem->item_type == "category") {
                ...
                if (!empty($menuItem->item_link)) {
                    ...
                    return "deleteItem('PageController/deletePagePost','" . $menuItem->item_id . "','" . trans("confirm_page") . "');";
                }
            }
        }
    }
...
if (!function_exists('getFontFamily')) {
    function getFontFamily($activeFonts, $key)
    {
...
if (!function_exists('getPopularPosts')) {
    function getPopularPosts($dateType, $langId, $latestCategoryPosts)
    {
        $model = new \App\Models\PostModel();
        $popularPosts = array();
        if ($dateType == "week") {
            $popularPosts = getCachedDataByLang('popular_posts_week', $langId);
            if (empty($popularPosts)) {
                $popularPosts = completePopularPosts($model->getPopularPosts('weekly', $langId), $langId, $latestCategoryPosts);
                setCacheDataByLang('popular_posts_week', $popularPosts, $langId);
            }
        } elseif ($dateType == "month") {
            $popularPosts = getCachedDataByLang('popular_posts_month', $langId);
            if (empty($popularPosts)) {
                $popularPosts = completePopularPosts($model->getPopularPosts('monthly', $langId), $langId, $latestCategoryPosts);
                setCacheDataByLang('popular_posts_month', $popularPosts, $langId);
            }
        } else {
            $popularPosts = getCachedDataByLang('popular_posts_all', $langId);
            if (empty($popularPosts)) {
                $popularPosts = $model->getPopularPosts('all', $langId);
                setCacheDataByLang('popular_posts_all', $popularPosts, $langId);
            }
        }
        return $popularPosts;
    }
...
if (!function_exists('setPostMetaTags')) {
    function setPostMetaTags($post, $postTags, $data)
    {
        ...
        $data['ogType'] = 'article';
        $data['ogImage'] = base_url($post->image_big);
        if (!empty($post->image_url)) {
            $data['ogImage'] = $post->image_url;
        }
        $data['ogWidth'] = '750';
        $data['ogHeight'] = '422';
