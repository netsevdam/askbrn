...
class AdminController extends BaseAdminController
{
    ...
    public function addMenuLinkPost()
    {
        ...
        $val->setRule('title', trans("title"), 'required|max_length[65]');
    ...
    public function adSpaces()
    {
        ...
        $data['adCodes'] = $this->commonModel->getAdCodes($data['adSpace']);
    ...
    public function adSpacesPost()
    {
        ...
        if ($this->commonModel->updateAdSpaces($adSpace)) {
            $this->session->setFlashdata('success', trans("msg_updated"));
        } else {
            $this->session->setFlashdata('error', trans("msg_error"));
        }
        return redirect()->to(adminUrl('ad-spaces?ad_space=' . esc($adSpace)));
    }
    ...
    public function addUserPost()
    {
        checkAdmin();
        $val = \Config\Services::validation();
        $val->setRule('username', trans("username"), 'required|max_length[65]');
        $val->setRule('email', trans("email"), 'required|max_length[65]');
        if (!$this->validate(getValRules($val))) {
            $this->session->setFlashdata('errors', $val->getErrors());
    ...
    public function visualSettingsPost()
    {
        ...
        return redirect()->to(adminUrl('visual-settings'));
    }
