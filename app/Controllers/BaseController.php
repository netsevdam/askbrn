...
class BaseController extends Controller
{
    ...
    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        ...
        //widgets
        $widgets = getCachedData('widgets');
        if (empty($widgets)) {
            $widgets = $this->settingsModel->getWidgetsByLang($this->activeLang->id);
            setCacheData('widgets', $widgets);
        }
        //categories
        ...
        //ad spaces
        $adSpaces = $this->commonModel->getAdSpaces();
        ...
        $view->setData(['activeLang' => $this->activeLang, 'generalSettings' => $this->generalSettings, 'baseSettings' => $this->settings, 'activeLanguages' => $this->activeLanguages, 'rtl' => $this->rtl,
            'darkMode' => $this->darkMode, 'activeFonts' => $this->activeFonts, 'baseMenuLinks' => $menuLinks, 'baseWidgets' => $widgets, 'baseCategories' => $this->categories, 'baseLatestCategoryPosts' => $this->latestCategoryPosts, 'adSpaces' => $adSpaces]);
    }
}
